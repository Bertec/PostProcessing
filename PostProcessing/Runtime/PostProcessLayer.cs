...
namespace UnityEngine.Rendering.PostProcessing
{
    ...
    public sealed class PostProcessLayer : MonoBehaviour
    {
        ...
        void BuildCommandBuffers()
        {
            ...
            bool vrSinglePassInstancingEnabled = context.stereoActive && context.numberOfEyes > 1 && (context.stereoRenderingMode == PostProcessRenderContext.StereoRenderingMode.SinglePassInstanced || context.stereoRenderingMode == PostProcessRenderContext.StereoRenderingMode.SinglePassMultiview);
        ...
        int RenderBuiltins(PostProcessRenderContext context, bool isFinalPass, int releaseTargetAfterUse = -1, int eye = -1)
        {
            ...
            else if (context.stereoActive && context.stereoRenderingMode == PostProcessRenderContext.StereoRenderingMode.SinglePassMultiview)
            {
                uberSheet.properties.SetFloat(ShaderIDs.DepthSlice, eye);
                cmd.BlitFullscreenTriangleToTexArray(context.source, context.destination, uberSheet, 0, false, eye);
            }
        ...
        void RenderFinalPass(PostProcessRenderContext context, int releaseTargetAfterUse = -1, int eye = -1)
        {
            ...
            if (breakBeforeColorGrading)
            {
                ...
                {
                    // Enable keyword for both instanced + multiview
                    if (context.stereoRenderingMode == PostProcessRenderContext.StereoRenderingMode.SinglePassInstanced || context.stereoRenderingMode == PostProcessRenderContext.StereoRenderingMode.SinglePassMultiview)
                        sheet.EnableKeyword("STEREO_INSTANCING_ENABLED");
