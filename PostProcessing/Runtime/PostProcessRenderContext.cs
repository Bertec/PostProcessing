...
namespace UnityEngine.Rendering.PostProcessing
{
...
#endif
    using UnityEngine.Rendering; // Added for GraphicsDeviceType
    ...
    public sealed class PostProcessRenderContext
    {
        ...
        public Camera camera
        {
            ...
            set
            {
                ...
                if (m_Camera.stereoEnabled)
                {
                    ...
                    {
                        // Tex2DArray can represent instanced single-pass (PC/Console) or multiview (Android).
                        // Heuristic: on Android (OpenGLES3/Vulkan) with SinglePass (not MultiPass) we assume multiview.
#if UNITY_ANDROID
                        if (stereoRenderingMode != StereoRenderingMode.MultiPass)
                        {
                            var gdt = SystemInfo.graphicsDeviceType;
                            if (gdt == GraphicsDeviceType.Vulkan || gdt == GraphicsDeviceType.OpenGLES3)
                        ...
                        {
                            stereoRenderingMode = StereoRenderingMode.SinglePassInstanced; // fallback
                        }
#else
                        stereoRenderingMode = StereoRenderingMode.SinglePassInstanced;
#endif
        ...
        public void UpdateSinglePassStereoState(bool isTAAEnabled, bool isAOEnabled, bool isSSREnabled)
        {
            ...
            else if (stereoRenderingMode == StereoRenderingMode.SinglePassMultiview || stereoRenderingMode == StereoRenderingMode.SinglePassInstanced)
            {
                // Multiview & instanced already supply per-eye layers in a texture array; keep numberOfEyes=2
                numberOfEyes = 2;
                width = xrDesc.width;
                height = xrDesc.height;
                m_sourceDescriptor = xrDesc;
